...
} = require('@feelhq/feel-transactions');
const { validator: feelValidator } = require('@feelhq/feel-validator');
...
class Application {
	...
	constructor(genesisBlock, config = {}) {
		const errors = feelValidator.validate(genesisBlockSchema, genesisBlock);
		if (errors.length) {
			throw errors;
		}
		...
		if (!_.has(appConfig, 'app.label')) {
			_.set(
				appConfig,
				'app.label',
				`feel-${genesisBlock.payloadHash.slice(0, 7)}`,
			);
		}
		...
		if (!_.has(appConfig, 'components.logger.logFileName')) {
			_.set(
				...
				`${process.cwd()}/logs/${appConfig.app.label}/feel.log`,
			);
		}
		...
		// TODO: This should be removed after https://github.com/feelfoundation/feel/pull/2980
	 ...
	 * @param {constructor} Transaction - Implementation of @feelhq/feel-transactions/base_transaction
	 */
	 ...
	 */
	async run() {
		this.logger.info(`Booting the application with Feel Framework(${version})`);
	...
	_compileAndValidateConfigurations() {
		...
		const childProcessModules = process.env.FEEL_CHILD_PROCESS_MODULES
			? process.env.FEEL_CHILD_PROCESS_MODULES.split(',')
			: [];
