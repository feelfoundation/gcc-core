...
const stopCommand = async (installDir, network, name) => {
    try {
        const { components: { cache: { password }, }, } = await config_1.getFeelConfig(installDir, network);
        const { redisPort } = (await pm2_1.describeApplication(name));
        if (password) {
...
exports.isCacheEnabled = async (installDir, network) => {
    try {
        const { components: { cache: { enabled }, }, } = await config_1.getFeelConfig(installDir, network);
        return enabled;
    }
    catch (error) {
        throw new Error(error);
