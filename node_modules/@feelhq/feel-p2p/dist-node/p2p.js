...
const feel_cryptography_1 = require("@feelhq/feel-cryptography");
...
exports.DEFAULT_RANDOM_SECRET = feel_cryptography_1.getRandomBytes(SECRET_BYTE_LENGTH).readUInt32BE(0);
...
class P2P extends events_1.EventEmitter {
    ...
    _handleEmit(req, next) {
        const MAX_EVENT_NAME_LENGTH = 128;
        if (req.event.length > MAX_EVENT_NAME_LENGTH) {
            this._terminateIncomingSocket(req.socket, `Terminated connection peer: ${req.socket.remoteAddress}, reason: Unsupported event name: ${req.event}, length ${req.event.length}, max supported event name length is: ${MAX_EVENT_NAME_LENGTH}`, true);
    ...
    _handleIncomingPayload(ws, _req) {
        ws.on('message', (message) => {
            ...
            try {
                ...
                validation_1.validateMessage(parsed);
                ...
                if (parsed.event === '#disconnect') {
                    ...
                    if (count > exports.DEFAULT_CONTROL_MESSAGE_LIMIT) {
                        throw new Error('Received invalid payload');
                    }
                }
            }
                ...
                ws.terminate();
                this._handleBanPeer(peerId);
                this.emit(peer_pool_1.EVENT_INBOUND_SOCKET_ERROR, `Banned peer with Ip: ${peerIpAddress}, reason: ${error}, message: ${message}`);
            }
        });
    }
    _handleIncomingHandshake(req, next) {
        if (this._sanitizedPeerLists.blacklistedPeers) {
            ...
            if (blacklist.includes(req.socket.remoteAddress)) {
                ...
                next(new errors_1.PeerInboundHandshakeError(disconnect_status_codes_1.FORBIDDEN_CONNECTION_REASON, disconnect_status_codes_1.FORBIDDEN_CONNECTION, req.socket.remoteAddress));
                return;
            }
        }
        if (this._bannedPeers.has(req.socket.remoteAddress)) {
            const existingPeer = this._peerBook
                .getAllPeers()
                .find(peer => peer.ipAddress === req.socket.remoteAddress);
            if (existingPeer) {
    ...
    _handleIncomingConnection(socket) {
        if (this._sanitizedPeerLists.blacklistedPeers.find(peer => peer.ipAddress === socket.remoteAddress)) {
    ...
    async _startPeerServer() {
        ...
        this._scServer.addMiddleware(this._scServer.MIDDLEWARE_EMIT, (req, next) => {
    ...
    _handleGetPeersRequest(request) {
        if (!validation_1.isEmptyMessage(request.data)) {
            this.applyPenalty({
