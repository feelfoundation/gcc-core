...
exports.getReleaseInfo = async (releaseUrl, network, installVersion) => {
    if (releaseUrl && releaseUrl.search('.tar.gz') >= 0) {
        return {
            ...
            feelTarUrl: releaseUrl,
            feelTarSHA256Url: `${releaseUrl}.SHA256`,
        };
    }
    const version = installVersion;
    const urlPath = `${constants_1.RELEASE_URL}/${network}/${version}`;
    const feelTarUrl = `${urlPath}/${commons_1.feelTar(version)}`;
    const feelTarSHA256Url = `${urlPath}/${commons_1.feelTarSHA256(version)}`;
    return { version, feelTarUrl, feelTarSHA256Url };
};
