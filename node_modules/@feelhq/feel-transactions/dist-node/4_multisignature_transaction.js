...
const setMemberAccounts = (store, membersPublicKeys) => {
    membersPublicKeys.forEach(memberPublicKey => {
        const address = feel_cryptography_1.getAddressFromPublicKey(memberPublicKey);
...
class MultisignatureTransaction extends base_transaction_1.BaseTransaction {
    ...
    async prepare(store) {
        const membersAddresses = extractPublicKeysFromAsset(this.asset.multisignature.keysgroup).map(publicKey => ({ address: feel_cryptography_1.getAddressFromPublicKey(publicKey) }));
