...
const startPM2 = async (installPath, network, name, envConfig) => {
    const { apps } = await fs_extra_1.default.readJson(`${installPath}/etc/pm2-feel.json`);
    return new Promise((resolve, reject) => {
        pm2_1.start({
            ...
            cwd: installPath,
            env: Object.assign({ FEEL_NETWORK: network }, envConfig),
            pid: path.join(installPath, '/pids/feel.app.pid'),
            output: path.join(installPath, '/logs/feel.app.log'),
            error: path.join(installPath, '/logs/feel.app.err'),
...
const extractProcessDetails = (appDesc) => {
    ...
    const { status, pm_uptime, pm_cwd: installationPath, version, FEEL_NETWORK: network, FEEL_DB_PORT: dbPort, FEEL_REDIS_PORT: redisPort, FEEL_HTTP_PORT: httpPort, FEEL_WS_PORT: wsPort, } = pm2_env;
