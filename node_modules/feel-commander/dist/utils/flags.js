...
const messageDescription = `Specifies a source for providing a message to the command. If a string is provided directly as an argument, this option will be ignored. The message must be provided via an argument or via this option. Sources must be one of \`file\` or \`stdin\`. In the case of \`file\`, a corresponding identifier must also be provided.
	...
	- --message=stdin
`;
const passphraseDescription = `Specifies a source for your secret passphrase. Feel Commander will prompt you for input if this option is not set.
...
`;
const secondPassphraseDescription = `Specifies a source for your second secret passphrase. For certain commands a second passphrase is necessary, in which case Feel Commander will prompt you for it if this option is not set. Otherwise, Feel Commander will assume you want to use one passphrase only.
	Source must be one of \`prompt\`, \`pass\`, \`env\`, \`file\` or \`stdin\`. For \`pass\`, \`env\` and \`file\` a corresponding identifier must also be provided.
	Examples:
...
`;
const passwordDescription = `Specifies a source for your secret password. Feel Commander will prompt you for input if this option is not set.
	Source must be one of \`prompt\`, \`pass\`, \`env\`, \`file\` or \`stdin\`. For \`pass\`, \`env\` and \`file\` a corresponding identifier must also be provided.
	Examples:
...
const networkDescription = 'Feel Core network name.';
const installationPathDescription = 'Feel Core installation path.';
const releaseUrlDescription = 'Feel Core download URL.';
const snapshotUrlDescription = 'Feel Core blockchain snapshot URL.';
const noSnapshotDescription = 'Install Feel Core without a blockchain snapshot.';
const feelVersionDescription = 'Feel Core version.';
const noStartDescription = 'Install Feel Core without starting.';
exports.flags = {
    message: {
        char: 'm',
    ...
    feelVersion: {
        description: feelVersionDescription,
    },
    noStart: {
