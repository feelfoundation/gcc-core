...
exports.verifyBalance = (id, account, amount) => new BigNum(account.balance).lt(new BigNum(amount))
    ? new errors_1.TransactionError(`Account does not have enough F39: ${account.address}, balance: ${format_1.convertFellowsToF39(account.balance.toString())}`, id, '.balance')
    : undefined;
exports.verifyAmountBalance = (id, account, amount, fee) => {
    ...
    if (balance.gte(0) && balance.lt(new BigNum(amount))) {
        return new errors_1.TransactionError(`Account does not have enough F39: ${account.address}, balance: ${format_1.convertFellowsToF39(balance.plus(fee).toString())}`, id, '.balance');
    }
    return undefined;
};
