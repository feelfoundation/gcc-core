...
Modules are individual building blocks for Feel Core.

### Core Modules

Core Modules are shipped along with the Feel Core distribution itself. These modules constitute the minimum requirements to run a functional Feel Core instance.

#### List of Core Modules

- **Chain Module:** handles all events and actions, that are related to the blockchain system.
- **HTTP API Module:** provides API endpoints, that enable users and other programs to communicate with the Feel blockchain through the API.
...
Custom Modules can be plugged into Feel Core and may offer new features/capabilities for the application, or replace Core modules functionalities.
...
Currently, the only Feel native module supported is HTTP API module which will be loaded as child process if you have `ipc` enabled.
