...
class StatusCommand extends base_1.default {
    async run() {
        ...
        if (name) {
            ...
            if (!instance) {
                throw new Error(`Feel Core instance: ${name} doesn't exists, Please install using feel core:install --help`);
            }
            this.print(instance);
        }
        else {
            ...
            if (!instances.length) {
                this.print({
                    message: 'Feel Core instances not available, Please install using feel core:install --help',
                });
            }
            else {
                const toDisplay = [
...
StatusCommand.args = [
    {
        name: 'name',
        description: 'Feel Core installation directory name.',
        required: false,
    },
];
StatusCommand.description = 'Show the status of a Feel Core instances.';
