...
class BaseTransaction {
    constructor(rawTransaction) {
        ...
        try {
            this._senderId = tx.senderId
                ? tx.senderId
                : feel_cryptography_1.getAddressFromPublicKey(this.senderPublicKey);
        }
        catch (error) {
            this._senderId = '';
        }
    ...
    getBytes() {
        const transactionBytes = Buffer.concat([
            this.getBasicBytes(),
            this._signature ? feel_cryptography_1.hexToBuffer(this._signature) : Buffer.alloc(0),
            this._signSignature ? feel_cryptography_1.hexToBuffer(this._signSignature) : Buffer.alloc(0),
        ]);
    ...
    sign(passphrase, secondPassphrase) {
        const { address, publicKey } = feel_cryptography_1.getAddressAndPublicKeyFromPassphrase(passphrase);
        ...
        this._signature = feel_cryptography_1.signData(feel_cryptography_1.hash(this.getBytes()), passphrase);
        if (secondPassphrase) {
            this._signSignature = feel_cryptography_1.signData(feel_cryptography_1.hash(this.getBytes()), secondPassphrase);
        }
    ...
    getBasicBytes() {
        ...
        const transactionSenderPublicKey = feel_cryptography_1.hexToBuffer(this.senderPublicKey);
        const transactionRecipientID = this.recipientId
            ? feel_cryptography_1.intToBuffer(this.recipientId.slice(0, -1), constants_1.BYTESIZES.RECIPIENT_ID).slice(0, constants_1.BYTESIZES.RECIPIENT_ID)
    ...
    _verify(sender) {
        const secondSignatureTxBytes = Buffer.concat([
            this.getBasicBytes(),
            feel_cryptography_1.hexToBuffer(this.signature),
        ]);
        return [
