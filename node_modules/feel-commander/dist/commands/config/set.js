...
const URL_ERROR_MESSAGE = `Node URLs must include a supported protocol (${constants_1.API_PROTOCOLS.map(protocol => protocol.replace(':', '')).join(', ')}) and a hostname. E.g. https://127.0.0.1:4500 or http://localhost.`;
...
const setNethash = (config, dotNotation, value) => {
    if (dotNotation === 'api.network' &&
        ...
        try {
            feel_cryptography_1.hexToBuffer(value, 'utf8');
        }
        catch (error) {
            throw new error_1.ValidationError(NETHASH_ERROR_MESSAGE);
        }
...
SetCommand.examples = [
    ...
    'config:set api.nodes https://127.0.0.1:4500,http://mynode.com:7500',
];
