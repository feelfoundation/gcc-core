...
exports.createUser = async (installDir, network, name) => {
    try {
        const { components: { storage: { user, password }, }, } = await config_1.getFeelConfig(installDir, network);
        const { dbPort } = (await pm2_1.describeApplication(name));
...
exports.createDatabase = async (installDir, network, name) => {
    try {
        const { components: { storage: { database }, }, } = await config_1.getFeelConfig(installDir, network);
        const { dbPort } = (await pm2_1.describeApplication(name));
...
exports.restoreSnapshot = async (installDir, network, snapshotFilePath, name) => {
    try {
        const { components: { storage: { database, user }, }, } = await config_1.getFeelConfig(installDir, network);
        const { dbPort } = (await pm2_1.describeApplication(name));
