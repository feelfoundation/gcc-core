...
exports.getNetgroup = (address, secret) => {
    ...
    return feel_cryptography_1.hash(netgroupBytes).readUInt32BE(0);
};
exports.getBucket = (options) => {
    ...
    if (network !== NETWORK.NET_IPV4) {
        return (feel_cryptography_1.hash(Buffer.concat([secretBytes, networkBytes])).readUInt32BE(0) %
            secondMod);
    }
    ...
    const k = peerType === PEER_TYPE.NEW_PEER
        ? feel_cryptography_1.hash(Buffer.concat([
            secretBytes,
            networkBytes,
            targetABytes,
            targetBBytes,
        ])).readUInt32BE(0) % firstMod
        : feel_cryptography_1.hash(Buffer.concat([secretBytes, networkBytes, addressBytes])).readUInt32BE(0) % firstMod;
    ...
    return feel_cryptography_1.hash(bucketBytes).readUInt32BE(0) % secondMod;
};
